...
`src/game/audio.ts` 使用 Web Audio API 创建原创的 D 小调背景音乐和机械/后备音效。狙击枪射击声复用 CC0 步枪音效降低播放速率；命中区域和梯子变体复用 CC0 撞击音效调整音高/增益。程序化音乐是原创项目内容，未使用任何来自 Project IGI 或其他游戏的录音、旋律或音效包。

## 已移除的内容

**Project IGI 录音（`igi/` 文件夹）已从本项目中完全移除**，因为这些音频资源未获得上游项目之外的重新使用授权。游戏现在完全依赖上述 CC0/CC BY 音效和程序化音频。角色对话现在仅显示字幕，不播放语音。

上游项目 README 和原始 CREDITS.md 明确指出 Project I.G.I. 录音不可重新使用。详见：https://github.com/byteab/operation-ink
