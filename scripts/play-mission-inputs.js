  ...
  const loop = now => {
    ...
    if(runner.combat){
      ...
      if(targets[0] && (e.mission.weapons.current?.magazine || e.mission.weapons.current?.reserve)){
        ...
        else if(now-lastShot>285){lastShot=now;const canvas=document.querySelector('#world');canvas.dispatchEvent(new MouseEvent('mousedown',{button:0,bubbles:true}));window.dispatchEvent(new MouseEvent('mouseup',{button:0,bubbles:true}));}
        return;
      }
    }
