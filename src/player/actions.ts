...
export class PlayerActions {
  ...
  findTarget(camera: THREE.Camera): ActionTarget | null {
    ...
    for (const door of this.doors) {
      ...
      consider({ object: door, point, kind: 'door', label: door.userData.open ? '关闭' : '打开', descending: false })
    }
    for (const ladder of this.ladders) for (const descending of [false, true]) {
      ...
      consider({ object: ladder, point, kind: 'ladder', label: descending ? '向下爬' : '向上爬', descending })
    }
