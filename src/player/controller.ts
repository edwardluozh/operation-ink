...
export class FirstPersonController {
  ...
  update(dt: number) {
    ...
    const state = ride ? `前往 ${ride.destination} · ${Math.round((1 - ride.remaining / ride.distance) * 100)}%` :
      this.actions.climbing ? (this.actions.climbing.descending ? '向下爬' : '向上爬') :
      !this.body.grounded ? '空中' : this.direction.lengthSq() > 0 ?
        (this.pressed.has('ShiftLeft') || this.pressed.has('ShiftRight') ? '冲刺中' : '行走中') : '徒步中'
    this.status.textContent = this.fallback ? `${state} · 拖动查看` : state
    return true
  }
