...
export class FirstPersonWeapons {
  ...
  pickupTargets() {
    return [...this.loose.values()].map(({ item, model }) => ({
      ...
      label: `${this.inventory.every(Boolean) ? '更换' : '拾取'} ${WEAPON_RULES[item.name].label}`, id: item.id,
    }))
  }
  ...
  pickup(id: string) {
    ...
    this.switchTime = 0.22
    this.setHeldModel()
    this.context.emit({ kind: 'pickup', position: this.feet.clone(), radius: 2, text: `已拾取 ${this.label}` })
    return true
  }
