...
export class MissionHUD {
  ...
  constructor(world: MissionWorld, callbacks: { retry: () => void; restart: () => void; volume: (value: number) => void; mute: (value: boolean) => void }) {
    ...
    document.title = i18n.gameTitle + ' — Stickman'
    const $ = <T extends HTMLElement = HTMLElement>(selector: string) => document.querySelector<T>(selector)!
    this.start = $<HTMLButtonElement>('#walk-start')
    this.start.textContent = i18n.loadingCompound; this.start.disabled = true
    $('.walk-heading .walk-eyebrow').textContent = i18n.gameTitle
    $('#world').setAttribute('aria-label', i18n.gameTitle + ' 战术任务。鼠标查看，WASD 移动，左键射击，右键切换瞄准，F 互动，R 重新装填，M 任务地图，Esc 暂停。')
  ...
  private buildMap(world: MissionWorld) {
    ...
    const stationNames: Record<string,string> = { cameras: i18n.locations.security, gate: i18n.locations.exitGate, jeep: i18n.locations.jeep, hostage: i18n.locations.cells, alarm: '警报', rally: '集合', supply: '补给', distraction: '警铃' }
    ...
    return `<svg viewBox="0 0 460 260" role="img" aria-label="Field map: north is up. Rail route via water tower; service route via warehouse and workshop. East annex holds underground detention, security, jeep and exit gate." stroke-linecap="round" stroke-linejoin="round">
      ...
      ${buildingsInk}
      <text x="${x(-43)}" y="${z(-59)}">${i18n.locations.messHall}</text><text x="${x(-92)}" y="${z(-50)}">${i18n.locations.serviceGate}</text><text x="${x(7)}" y="${z(5)}">${i18n.locations.warehouse}</text><text x="${x(64)}" y="${z(3)}">${i18n.locations.workshop}</text><text x="${x(132)}" y="${z(15)}">${i18n.locations.barracks}</text><text x="${x(108)}" y="${z(-18)}">${i18n.locations.detention}</text>
      ${world.stations.filter(s => ['cameras', 'gate', 'jeep'].includes(s.kind)).map(s => `<circle cx="${x(s.point.x)}" cy="${z(s.point.z)}" r="2.6" fill="var(--ink)"/><text text-anchor="${s.kind === 'gate' ? 'end' : 'start'}" x="${x(s.point.x)+(s.kind === 'gate' ? -5 : 5)}" y="${z(s.point.z)-5}">${s.id === SIGNALS_COMPUTER_ID ? i18n.locations.officeTerminal : stationNames[s.kind]}</text>`).join('')}
