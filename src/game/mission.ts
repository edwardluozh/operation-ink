...
export function missionObjective(state: MissionState) {
  if (state.phase === 'dead') return '营救中断。重试插入检查点。'
  if (state.phase === 'complete') return '人质已撤离。任务完成。'
  if (state.jeep === 'escaping') return '逃离营地'
  if (releasedCount(state) < state.hostages.length) {
    if (!state.detentionFound) return '在东侧附属区找到拘留所'
    if (!state.cellsReached) return '到达地下牢房'
    return '释放 01 号牢房中的人质'
  }
  if (loadedCount(state) < state.hostages.length) return '护送人质到吉普车'
